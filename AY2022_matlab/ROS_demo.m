...
%% ROS
%rosinit , type that into command window  in matlab
%mmatlab doesnt know how to use ROS so you type that into command prompt to initiate a ros node
matlab_listens = rossubscriber('/finalTesting') %tells you which ros node to use
pause(2)
tic
while toc < 5
    listener_msg = matlab_listens.LatestMessage.Data; %gives system 5 seconds to get info from ROS
    pause(0.1)
end
point = listener_msg
cmd_move.position = [NaN NaN NaN NaN NaN NaN]
...
t = toc;
%% Turn on if using ROS
points = reshape(point(1:end-1),3,[])';
nine = find(point == 9999);
tags = point(nine+1:end);
% destination = reshape(tags(1:end),3,[])';
% destination = destination(:,1:2);
% destination = [destination(:,2) destination(:,1)]
...
for i = 1:size(points,1)
    ...
    if i == 1
        z(2) = z(2)-0.035; %may have to adjust this based on lighting conditions
    end
    z(1) = z(1) + 0.035; %may have to adjust this based on lighting conditions
    
    z(3) = 0.37; %fixed height to move to round
