...
def StraightLine(startA,end,tol,numP):
    ...
    if(np.shape(end)[0]<12):
        #linPoints produces three matrices of  x,y, and z coordinates of desired waypoints
...
def addGimbal(ang, XYZ, tol, numP):
    ...
    for col in range(np.shape(newPos)[1]):
        newPos[1,col]=newPos[1,col]*-1
    for col in range(np.shape(newPos)[1]):
        newPos[2,col]=newPos[2,col]-(0.5*np.pi)  
    for col in range(np.shape(newPos)[1]):
        newPos[3,col]=newPos[3,col]*-1
    for col in range(np.shape(newPos)[1]):
