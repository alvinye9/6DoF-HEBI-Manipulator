...
from InvK_test import StraightLineAlt
from invKJustPos import StraightLine, addGimbal

def getTrajectory(startAngles, endXYZ, max_waypoints, speed): 
    pos = StraightLineAlt(startAngles,endXYZ, 0.01, 5)   #0.01 rad tolerance, 5 cuts to straight line trajectory
...
group = lookup.get_group_from_names([family_name], [mod8_name,mod6_name, mod5_name, mod3_name, mod2_name, mod1_name])
...
Startangles = np.matrix([[angles[0]],[angles[1]],[angles[2]],[angles[3]],[angles[4]],[angles[5]]])

# Point1 = np.matrix([[0.5], [0.3], [0]]) #WORKS

Point1 = np.matrix([[0.5], [0.5], [0.5]]) 
...
trajCalcValues = getTrajectory(Startangles, Point1, max_waypoints, speed) #current pos to Point 10
