...
class ArUcoDetector:
    ...
    def __init__(self, dict_to_use):
        ...
        self.arucoDict = cv2.aruco.getPredefinedDictionary(self.ARUCO_DICT[dict_to_use])
        self.arucoParams = cv2.aruco.DetectorParameters()
        self.detector = cv2.aruco.ArucoDetector(self.arucoDict, self.arucoParams)


    def detect(self, image):
        result = self.detector.detectMarkers(image)
        return result


    @staticmethod
    def getImageWithMarkers(input_image, detect_res, embedded=False):
        ...
        if len(corners) > 0:
            ...
            for (markerCorner, markerID) in zip(corners, ids):
                ...
                # compute and the center (x, y)-coordinates of the ArUco marker
                cX = int((topLeft[0] + bottomRight[0]) / 2.0)
                cY = int((topLeft[1] + bottomRight[1]) / 2.0)

                # don't draw anything yet if it's embedded in an application
                if not embedded:
                    # draw the bounding box and center of the ArUCo detection
                ...
                markerCorners.append(markerCorner)
                markerIDs.append(markerID)
                points.append((topRight, bottomRight, bottomLeft, topLeft, cX, cY))
            return image, markerCorners, markerIDs, points
...
if __name__ == '__main__':
    ...
    while True:
        ...
        color_image, mc, id, location = ArUcoDetector.getImageWithMarkers(color_image, result)
        masked_color_image, mc, id, location = ArUcoDetector.getImageWithMarkers(masked_color_image, result)
        depth_colormap, mc, id, location = ArUcoDetector.getImageWithMarkers(depth_colormap, result)
        ...
        # cv2.drawFrameAxes(color_image, cameraMatrix=, distCoeffs=, rvec=, tvec=, 0.1)
